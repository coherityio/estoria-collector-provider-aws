...
/**
 * Collects S3 bucket policies for all buckets.
 */
@Slf4j
public class S3BucketPolicyCollector extends AbstractAwsContextAwareCollector
{
    ...
    {
        super(awsCollectorInfoBuilder(ENTITY_TYPE, Set.of("S3Bucket"), Set.of("storage", "s3", "policy", "aws")).build());
        log.debug("S3BucketPolicyCollector created");
    }
        ...
        CollectorRequestParams collectorRequestParams) throws CollectorException
    {
            ...
            if (buckets != null)
            {
                for (Bucket bucket : buckets)
                {
                    ...
                    catch (S3Exception ex)
                    {
                        ...
                        {
                            log.debug("S3BucketPolicyCollector no policy for bucket: {}", bucketName);
                            continue;
                        }
                        log.warn("S3BucketPolicyCollector error fetching policy for {}: {}", bucketName, ex.getMessage());
                        continue;
                    }

                    if (policyJson == null || policyJson.isBlank()) continue;

                    String bucketArn = ARNHelper.s3BucketArn(bucketName);
                    String id = bucketArn + "/policy";

                    Map<String, Object> attributes = new HashMap<>();
                    attributes.put("bucketName", bucketName);
                    attributes.put("bucketArn", bucketArn);
                    attributes.put("policy", policyJson);
        ...
        {
            log.error("S3BucketPolicyCollector S3 error: {}",
                e.awsErrorDetails() != null ? e.awsErrorDetails().errorMessage() : e.getMessage(), e);
            throw new CollectorException("Failed to collect S3 bucket policies", e);
        }
        catch (Exception e)
        {
            log.error("S3BucketPolicyCollector unexpected error", e);
            throw new CollectorException("Unexpected error collecting S3 bucket policies", e);
        }
    }
}
