...
/**
 * Collects Application Load Balancers (ALB) via the ELBv2 DescribeLoadBalancers API,
 * filtered to type=application.
 */
@Slf4j
public class ApplicationLoadBalancerCollector extends AbstractAwsContextAwareCollector
{
    public static final String ENTITY_TYPE = "ApplicationLoadBalancer";


    public ApplicationLoadBalancerCollector()
    {
        super(awsCollectorInfoBuilder(ENTITY_TYPE, Set.of(), Set.of("loadbalance", "alb", "application", "aws")).build());
        log.debug("ApplicationLoadBalancerCollector created");
    }
    ...
    {
        log.debug("ApplicationLoadBalancerCollector.collect called");
        ...
        try
        {
            ...
            collectorRequestParams.getCursorToken().ifPresent(token -> {
                log.debug("ApplicationLoadBalancerCollector resuming from nextMarker: {}", token);
            ...
            List<LoadBalancer> lbs = response.loadBalancers().stream()
                .filter(lb -> LoadBalancerTypeEnum.APPLICATION.equals(lb.type()))
                .collect(Collectors.toList());

            log.debug("ApplicationLoadBalancerCollector received {} ALBs, nextMarker={}", lbs.size(), nextMarker);
        ...
        {
            log.error("ApplicationLoadBalancerCollector ELBv2 error: {}",
                e.awsErrorDetails() != null ? e.awsErrorDetails().errorMessage() : e.getMessage(), e);
            throw new CollectorException("Failed to collect application load balancers", e);
        }
        catch (Exception e)
        {
            log.error("ApplicationLoadBalancerCollector unexpected error", e);
            throw new CollectorException("Unexpected error collecting application load balancers", e);
        }
    }
}
